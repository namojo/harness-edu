...
## 1. `codex review` — 교차 검증을 한 줄로

구현자(사람 또는 다른 에이전트)가 아닌 **분리된 눈**으로 변경을 검토합니다.

```bash
# 아직 커밋 전이라면 언커밋 변경을, 브랜치 작업이면 base 대비를 리뷰
codex review --uncommitted | tee review.md
# 또는
codex review --base main --title "품질 스크립트 점검" | tee review.md
```

리뷰만 필요하므로 별도 샌드박스 지정 없이 안전하게 읽기만 합니다. `codex exec`와 달리
**리뷰 포맷이 정형화**돼 있어 발견 항목이 일관된 형태로 나옵니다. 결과는 stdout으로
흐르니 `tee`로 저장하세요.

**관찰 포인트**: 테스트가 통과했는데도 리뷰가 문제를 지적한다면, "테스트 통과 ≠ 정확"이
증명된 것입니다. 이것이 검증 단계를 테스트와 **별도로** 두는 이유입니다.

---

## 2. `codex exec` — 진단부터 패치까지

### 2.1 읽기 전용 진단 (안전 기본값)

먼저 권한을 주지 않고 무엇이 문제인지만 물어봅니다.

```bash
codex exec -p review \
  --output-last-message /tmp/diag.md \
  "src/quality_check.py 를 검토해라. 테스트는 통과하지만 불량률 계산과
   경계 처리에 결함이 있는지, 있다면 무엇인지 근거와 함께 진단해라. 수정은 하지 마라."
```

`-p review` 프로파일(`.codex/config.example.toml` 참고)이 `read-only`를 적용하므로
Codex는 파일을 못 고칩니다 — 진단만 받습니다.

### 2.2 쓰기 권한을 주고 패치

진단이 타당하면 그때 쓰기 권한을 줍니다.

```bash
codex exec -p patch \
  --output-last-message /tmp/patch.md \
  "src/quality_check.py 의 불량률 계산 단위 오류와 0 나눗셈 경계를 수정하고,
   tests/ 에 절대값·경계값 테스트를 추가해 실제로 결함을 잡도록 강화한 뒤
   pytest 로 통과를 확인해라. 변경 요약을 남겨라."
```

`-p patch`는 `workspace-write` — 작업 루트 안에서만 씁니다. 비대화형이라 사람에게 묻지
않으므로, 끝난 뒤 diff를 사람이 검토하는 흐름을 유지하세요.
...
**② 권한은 단계적으로 준다.** 먼저 `read-only`로 진단만 받고(2.1), 진단이 타당할 때만
`workspace-write`로 패치(2.2). "일단 고쳐 봐"보다 안전하고, 사람이 개입할 지점이 생깁니다.
